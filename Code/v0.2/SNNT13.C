...
static const int N_TrackingLayers     = 8;
static const int MaxStreams           = MaxNeurons + N_TrackingLayers;
static const int MaxStrips            = 256;
...
static double phivalues[MaxClasses];           // Phi angle (atan(x/y)) for class
...
static double strip_y[MaxStrips];               
static double strip_x[N_TrackingLayers];
static int Strip_first;
...
static const double a_plus     = 0.03125/2;   // for model of EPSP
static const double a_minus    = 0.0265625/2; // 0.85*a_plus;
...
void Simulate_bgrhits (double bgr_rate) {
    for (int itl=0; itl<N_TrackingLayers; itl++) {
        for (int is=0; is<N_strips; is++) {
            if (myRNG->Uniform()<bgr_rate) hit[itl][is] = 1; 
        }
    }
    return;
}
...
int Simulate_sighits (int pclass) {
    // We first compute xc, yc given x0, y0, p, phi, charge
    ...
    double x0    = StartLayer*width; // position vertex of track pairs on third layer if KoS=1
    double y0    = myRNG->Uniform(pitch*N_strips);
    double p     = pvalues[pclass]; 
    double phi   = phivalues[pclass]; 
    double ch    = charge[pclass];
    double R     = p*ch/(0.3*Bfield)*100.;  // we transfer particle charge to sign of radius (in cm)
    double denom = sqrt(1.+pow(tan(phi),2)); 

    // Now we can compute the position y at each layer given layer x
    int ismin    = N_strips-1;
    for (int itl=StartLayer; itl<N_TrackingLayers; itl++) { 
        double x_hit  = width*itl;
        for (int ich=0; ich<N_part; ich++) { 
            if (ich>0 && (KindOfSignal==1 || myRNG->Uniform()>0.5)) { // V-particles are +- pairs (of same momentum and phi, for now)
                ch = -ch;
                R  = -R;                
            }
            double xc     = R*tan(phi)/denom + x0; // x0 is 0 for KindOfSignal=0, =StartLayer*width for KindOfSignal=1
            double yc     = y0 + R/denom;
            ...
            if (insqrt>=0.) {
                ...
                cout << "Bad track: KoS=" << KindOfSignal << " pclass=" << pclass << " StartLayer=" << StartLayer << " p=" << p << " phi=" << phi << endl;
                cout << "Itl = " << itl << " xhit=" << x_hit
                     << " R=" << R << " xc,yc=" << xc << "," << yc << " insqrt=" << insqrt << endl;
            }
        }
    }
    Strip_first = ismin;
...
void Encode (double t_in) { 
    // One strip == one time step
    for (int is=0; is<N_strips; is++) {
        for (int itl=0; itl<N_TrackingLayers; itl++) {
            if (hit[itl][is]>0) {
                PreSpike_Time.push_back(t_in+is*TimeStep);
                PreSpike_Stream.push_back(itl);
                if (hit[itl][is]>1) { // signal hit
                    PreSpike_Signal.push_back(1);
                } else {              // background hit
                    PreSpike_Signal.push_back(0);
                }
            }
        }
    }
...
double Inhibitory_potential (double delta_t, int ilayer) {
    ...
    if (delta_t>=0. && delta_t<MaxDeltaT) ip = thisalpha * Spike_potential(delta_t, ilayer); 
    return ip;
}
...
double Compute_Selectivity(int level, int mode) {
    ...
    if (mode==0) { // Use additive rule
        ...
        // where Eff_i is the average efficiency of neuron i over classes, and Eff_j is the
...
void SNN_Tracking (int N_ev, int N_ep, int NL0, int NL1, int N_cl, double CF = 1., double Occ=0.00390625, 
    ...
    // N_ep:      number of weight-learning cycles
    ...
    // -----------------------------
    // Define tracker geometry
    // Initialize neuron potentials and synapse weights
    // Simulate background hits
    // Simulate hits from particle interactions 
    ...
    for (int ic=0; ic<N_classes; ic++) {
        pvalues[ic]   = 1.0 + 1.0*(ic/6);
        phivalues[ic] = -0.05 + 0.05*((ic%6)/2);
        charge[ic]    = pow(-1.,ic%2);
    }
    ...
    do {
        ...
        for (int ispike=0; ispike<PreSpike_Time.size(); ispike++) { 
            ...
            for (auto in : neurons_index)
            {
                ...
                if (is < N_TrackingLayers || Neuron_layer[in] > 0)
